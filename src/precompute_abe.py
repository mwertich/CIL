...
class AugmentationBasedExpert(nn.Module):

    def __init__(self, model, transformation, inv_transformation=None, uq=True):
        super(AugmentationBasedExpert, self).__init__()
        self.transformation = transformation
        self.model = model
        self.uq = uq
        if inv_transformation is None:
            self.inv_transformation = nn.Identity()
        else:
...
def predict_model(model, train_loader, val_loader, test_loader, out_dir, eps=1e-8):
    model.eval()
    with torch.no_grad():
...
def main(config):
    ...
    base_model = MiDaSUQ(backbone="vitl16_384")
    base_model.to(device)
    base_model.load_state_dict(torch.load(config.base_model_path, map_location=device), strict=False)
    
    transformations = [
        lambda x: TF.adjust_sharpness(x, 2),
        [lambda x: TF.hflip(x), lambda y: TF.hflip(y)],
        [lambda x: TF.rotate(x, 90), lambda y: TF.rotate(y, 270)],
        [lambda x: TF.rotate(x, 180), lambda y: TF.rotate(y, 180)],
        [lambda x: TF.rotate(x, 270), lambda y: TF.rotate(y, 90)],
    ]

    expert_models = []
    for transformation in transformations:
        if isinstance(transformation, Iterable):
            expert = AugmentationBasedExpert(base_model, transformation[0], transformation[1])
        else:
            expert = AugmentationBasedExpert(base_model, transformation)
